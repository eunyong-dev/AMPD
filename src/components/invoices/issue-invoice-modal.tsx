...
export function IssueInvoiceModal({
  ...
  useEffect(() => {
    ...
    // 동일 settlement에 이미 발행된 인보이스가 있는지 확인
    (async () => {
      setChecking(true);
      try {
        ...
        if (data && data.length > 0) {
          setExistingInvoice(data[0] as InvoiceRow);
        }
      } catch (err) {
  ...
  // invoiceDate 변경 시 dueDate 자동 갱신 (사용자가 따로 만지지 않은 경우)
  useEffect(() => {
    if (!invoiceDate) return;
    setDueDate((prev) => {
      // dueDate가 이전 invoiceDate + 30일 패턴이면 다시 갱신
      // 단순 처리: 항상 invoiceDate + 30일로 갱신
    ...
    <Dialog open={isOpen} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className='max-w-md'>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleIssue} disabled={submitting || checking}>
            {submitting ? '발행 중...' : '발행'}
          </Button>
        </DialogFooter>
      </DialogContent>
