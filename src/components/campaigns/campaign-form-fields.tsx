      ...
      <Popover>
        <PopoverTrigger asChild>
          ...
          >
            ...
            {/* X 클리어 — required 아니고 값 있을 때만 */}
            {canClear && (
              <span
                role='button'
                tabIndex={0}
                onPointerDown={(e) => {
                  // Popover 가 열리지 않도록 trigger 의 pointerDown 차단
                  e.preventDefault();
                  e.stopPropagation();
                }}
                onClick={(e) => {
                ...
                className='ml-auto inline-flex h-5 w-5 items-center justify-center rounded-md text-muted-foreground hover:bg-muted hover:text-foreground transition-colors'
            ...
            <CalendarIcon
              className={`${canClear ? '' : 'ml-auto'} h-4 w-4 opacity-50`}
            />
          </Button>
        </PopoverTrigger>
